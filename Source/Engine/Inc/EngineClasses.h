...
{
public:
    BYTE MoverEncroachType GCC_ALIGN(4); /* Keep out of ABrush's tail padding so the C++ layout matches script (UStruct::Link starts derived properties at the aligned base size). */
    BYTE MoverGlideType;
    BYTE BumpType;
    BYTE KeyNum;
    BYTE PrevKeyNum;
...
{
public:
    PTRINT Context; // Really a FMD5Context* (see AStatLog.cpp); script side widened via GetPlaceholderSize.
...
public:
    ...
    PTRINT LogAr GCC_PACK(4); // Really a FArchive* (see AStatLog.cpp); script side widened via GetPlaceholderSize.
...
{
public:
    BYTE TeamNumber GCC_ALIGN(4); /* Keep out of ANavigationPoint's tail padding so the C++ layout matches script (UStruct::Link starts derived properties at the aligned base size). */
