...
public:
    ...
    INT Port;
    INT RemoteSocket;
    PTRINT PrivateResolveInfo; // Really a FResolveInfo* (see GetResolveInfo); script side widened via GetPlaceholderSize.
    INT DataPending;
    BYTE ReceiveMode;
...
{
public:
    BYTE LinkState GCC_ALIGN(4); /* Keep out of AInternetLink's tail padding so the C++ layout matches script (UStruct::Link starts derived properties at the aligned base size). */
    FIpAddr RemoteAddr;
